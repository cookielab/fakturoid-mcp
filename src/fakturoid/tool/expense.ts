...
import { createCreateExpenseSchema, createUpdateExpenseSchema, GetExpenseFiltersSchema } from "../model/expense.js";
import { createTool, type ServerToolCreator } from "./common.js";

export const createExpenseTools = (context: ServerContext): ServerToolCreator[] => {
	...
	const createExpense = createTool(
		...
		async (client, expenseData) => {
			...
			if (expenseData.attachments) {
				...
				for (const attachment of expenseData.attachments) {
					const processed = await processAttachment(attachment, context, client);
					processedAttachments.push(processed);
				}
				expenseData.attachments = processedAttachments;
			}
		...
		createCreateExpenseSchema(context).shape,
	);
	...
	const updateExpense = createTool(
		...
		{
			id: z.number(),
			updateData: createUpdateExpenseSchema(context),
		},
	);
